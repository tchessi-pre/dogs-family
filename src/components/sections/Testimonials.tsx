...
import { ChevronLeft, ChevronRight, PenSquare, Star } from "lucide-react";

import { GOOGLE_REVIEWS_META, TESTIMONIALS } from "@/lib/data";
...
export function Testimonials() {
  ...
  return (
    <section
      id="temoignages"
      className="bg-plum-100 py-24 sm:py-32"
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-sm leading-relaxed text-plum-700 sm:text-base">
            Des retours authentiques laissés directement sur Google par des
            clients accompagnés par Léa.
          </p>

          {/* Note Google compacte */}
          <div className="mt-6 inline-flex items-center gap-3 rounded-full bg-white px-5 py-2.5 shadow-sm ring-1 ring-plum-900/10">
            ...
            <span className="font-display font-semibold text-plum-900">
              {GOOGLE_REVIEWS_META.rating.toFixed(1)}
            </span>
            <span className="text-sm text-plum-400">
              · {GOOGLE_REVIEWS_META.total} avis
            </span>
          </div>
        </div>
              ...
              {Array.from({ length: pageCount }).map((_, i) => (
                <span
                  ...
                  className={cn(
                    ...
                    currentPage === i ? "w-6 bg-plum-900" : "w-1.5 bg-plum-300",
                  )}
                />
              ))}
            </div>
          ...
          <div className="mt-8 hidden items-center justify-center gap-1.5 lg:flex">
            {Array.from({ length: pageCount }).map((_, i) => (
              <button
                ...
                onClick={() => setOrder(rotate(TESTIMONIALS, i * 3))}
                className={cn(
                  "h-1.5 rounded-full transition-all duration-300",
                  currentPage === i
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <a
            ...
            className={cn(
              ...
              "rounded-full border-plum-900/15 bg-white px-6 text-plum-900 shadow-sm hover:bg-plum-50",
            )}
          >
          ...
          <GoogleMapsLink
            className={cn(
              ...
              "gap-2 rounded-full text-plum-500 hover:text-plum-900",
            )}
          >
